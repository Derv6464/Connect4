...
public final class Startup_sA1GYNviEeWsF7MALrkAZQ implements DIMEProcess {
	...
	private Startup_sA1GYNviEeWsF7MALrkAZQResult executeInternal(final Context ctx, final SIB_ID id) {
		...
		while (true) {
			switch (curr) {
				...
				case _93800WsuEe24R4KEeKcnxg: {
					curr = execute_93800WsuEe24R4KEeKcnxg(ctx);
					break;
				}
				default: {
	...
	public SIB_ID execute_PXMhsNzNEeWtAOZsxPLKSA(final Context ctx) {
		...
		if (result.isEmpty()) {
			...
			// branch 'success'
			return SIB_ID._93800WsuEe24R4KEeKcnxg;
		}
	}
	...
	public SIB_ID execute_93800WsuEe24R4KEeKcnxg(final Context ctx) {
		try {
			info.scce.dime.app.demo.Native.sayHello("Mark ");
		}
		catch (Exception e) {
			e.printStackTrace();
			// branch 'failure'
